        ...
        for i in range(to_cut):
            ...
            for i, size in enumerate(resultant_sizes):
                ...
                detail = {
                    ...
                    'diferencia': f'{size}',
                    'medir_en': 'cm',
                    'kilos': round((resultant_sizes[i]*peso_promedio/source_size)/to_cut,2),
                    'destino': 'Cliente',
                }
            ...
            if source_size != sum:
                detail = {
                    ...
                    'medir_en': 'cm',
                    'kilos': round(((source_size-sum)*peso_promedio/source_size)/to_cut,2),
                    'destino': 'Merma',  # Generating Destino A, Destino B, etc.
                }
                line_details.append((0, 0, detail))
    ...
    def action_confirm(self):
        ...
        for move in self.production_id.move_raw_ids:
            if move.product_id.product_cosal in ('rollo', 'hoja'):
                data['supply'] = {
                    'family': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Familia").name or "--",
                    'subfamily': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Subfamilia").name or "--",
                    'type': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Tipo").name or "--",
                    'color': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Color").name or "--",
                    'weight': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Gramos").name or "--",
                    'size': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Ancho cm").name or "--",
                    'centro': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Centro").name or "--",
                    'diametro': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Diámetro").name or "--",
                    'certificado': move.product_id.product_template_attribute_value_ids.filtered(lambda a: a.attribute_id.name == "Certificado").name or "--",
                }
                break
